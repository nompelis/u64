...
## 8. Caveats: persistence format

Version 2 of the library writes a versioned persistence format that is not compatible with files generated by prior versions.

Each file starts with a header containing the magic bytes `u64v2`, followed by a 16-bit big-endian version field. The high 8 bits store the major version and the low 8 bits store the minor version; this format is currently version `2.0`, encoded as `0x0200`.

After the header, the persistence format stores the record count as a 32-bit big-endian integer and stores keys and value sizes as 64-bit big-endian integers. Record values are stored as raw bytes.
